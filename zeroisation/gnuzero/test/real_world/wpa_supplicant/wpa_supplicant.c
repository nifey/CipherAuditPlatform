...
// static void wpa_bss_tmp_disallow_timeout(void *eloop_ctx, void
// *timeout_ctx); #if defined(CONFIG_FILS) && defined(IEEE8021X_EAPOL) static
// void wpas_update_fils_connect_params(struct wpa_supplicant *wpa_s); #endif
// /* CONFIG_FILS && IEEE8021X_EAPOL */ #ifdef CONFIG_OWE static void
// wpas_update_owe_connect_params(struct wpa_supplicant *wpa_s); #endif /*
// CONFIG_OWE */
...
/* BEGIN INCLUDE FROM src/common/defs.h */
enum wpa_alg
{
  WPA_ALG_NONE,
  WPA_ALG_WEP,
...
/* END INCLUDE FROM src/common/defs.h */

/* BEGIN INCLUDE FROM wpa_supplicant/config_ssid.h */
enum wpas_mode
{
  WPAS_MODE_INFRA = 0,
  WPAS_MODE_IBSS = 1,
...
/* END INCLUDE FROM wpa_supplicant/config_ssid.h */

/* BEGIN INCLUDE FROM wpa_supplicant/wpa_supplicant_i.h */
...
/* END INCLUDE FROM wpa_supplicant/wpa_supplicant_i.h */

extern void wpa_msg (void *, int, const char *, ...);
extern int wpa_drv_set_key (struct wpa_supplicant *, enum wpa_alg, const u8 *,
                            int, int, const u8 *, size_t, const u8 *, size_t,
                            enum key_flag);

/* END INCLUDE RELATED MODIFIED SECTION FOR GNUZERO */

int
wpa_supplicant_set_wpa_none_key (struct wpa_supplicant *wpa_s,
                                 struct wpa_ssid *ssid)
{
  u8 SCRUB_ATTR key[32];
  size_t keylen;
  enum wpa_alg alg;
  u8 seq[6] = { 0 };
  int ret;
...
//  *
//  * This function is used to schedule a timeout for the current
//  authentication
//  * attempt.
//  */
...
//  * wpa_supplicant_req_auth_timeout() and it is called when authentication
//  has
//  * been completed.
//  */
...
//  * @wpa_s: Pointer to wpa_supplicant data
//  *
//  * This function is used to configure EAPOL state machine based on the
//  selected
//  * authentication mode.
//  */
...
//  *
//  * This function is used to configure WPA state machine and related
//  parameters
...
// 	 * for works that have not been started. gas_query_free() will be
// unable
...
//  *
//  * This function clears the encryption keys that has been previously
//  configured
//  * for the driver.
//  */
...
// wpa_s->bgscan_ssid) 		wpa_supplicant_reset_bgscan(wpa_s); 	else if
// (state <
// WPA_ASSOCIATED) 		wpa_supplicant_stop_bgscan(wpa_s); #endif /*
// CONFIG_BGSCAN
// */
...
//  *
//  * This function can be used to request that the configuration data is
//  reloaded
//  * (e.g., after configuration file change). This function is reloading
//  * configuration only for one interface, so this may need to be called
//  multiple
...
// exiting", 			wpa_s->confanother); 		return -1;
// 	}
...
//  * @wpa_ie_len: Maximum wpa_ie buffer size on input. This is changed to be
//  the
//  * used buffer length in case the functions returns success.
//  * Returns: 0 on success or -1 on failure
//  *
//  * This function is used to configure authentication and encryption
//  parameters
//  * based on the network configuration and scan result for the selected BSS
//  (if
//  * available).
//  */
...
// 		wpa_dbg(wpa_s, MSG_DEBUG,
// 			"WPA: ssid proto=0x%x pairwise_cipher=0x%x
// group_cipher=0x%x key_mgmt=0x%x", 			ssid->proto,
// ssid->pairwise_cipher, ssid->group_cipher, 			ssid->key_mgmt);
// wpa_dbg(wpa_s, MSG_DEBUG, "WPA: BSS " MACSTR " ssid='%s'%s%s%s",
// MAC2STR(bss->bssid), 			wpa_ssid_txt(bss->ssid,
// bss->ssid_len), 			bss_wpa ? " WPA" : "",
// bss_rsn ? " RSN" : "", 			bss_osen ? " OSEN" : "");
// if (bss_rsn) { 			wpa_hexdump(MSG_DEBUG, "RSN", bss_rsn,
// 2 + bss_rsn[1]); 			if (wpa_parse_wpa_ie(bss_rsn, 2 +
// bss_rsn[1],
// &ie)) { 				wpa_dbg(wpa_s, MSG_DEBUG,
// "Could not parse RSN element"); 			} else { 				wpa_dbg(wpa_s,
// MSG_DEBUG, 					"RSN: pairwise_cipher=0x%x group_cipher=0x%x key_mgmt=0x%x",
...
// 			} else {
// 				wpa_dbg(wpa_s, MSG_DEBUG,
// 					"WPA: pairwise_cipher=0x%x
// group_cipher=0x%x
// key_mgmt=0x%x", 					ie.pairwise_cipher,
// ie.group_cipher, 					ie.key_mgmt);
// 			}
// 		}
// 		return -1;
// 	} else {
...
// 	wpa_dbg(wpa_s, MSG_DEBUG,
// 		"WPA: AP group 0x%x network profile group 0x%x; available group
// 0x%x", 		ie.group_cipher, ssid->group_cipher, sel);
// wpa_s->group_cipher = wpa_pick_group_cipher(sel); 	if (wpa_s->group_cipher
// < 0) { 		wpa_msg(wpa_s, MSG_WARNING, "WPA: Failed to select
// group " 			"cipher"); 		return -1;
// 	}
...
// pairwise 0x%x", 		ie.pairwise_cipher, ssid->pairwise_cipher,
// sel); 	wpa_s->pairwise_cipher = wpa_pick_pairwise_cipher(sel, 1);
// if
// (wpa_s->pairwise_cipher < 0) { 		wpa_msg(wpa_s, MSG_WARNING, "WPA:
// Failed to select pairwise " 			"cipher"); 		return
// -1;
// 	}
...
// key_mgmt 0x%x", 		ie.key_mgmt, ssid->key_mgmt, sel); 	if (0)
// { #ifdef CONFIG_IEEE80211R #ifdef CONFIG_SHA384 	} else if ((sel &
// WPA_KEY_MGMT_FT_IEEE8021X_SHA384) &&
// 		   os_strcmp(wpa_supplicant_get_eap_mode(wpa_s), "LEAP") != 0)
// { 		wpa_s->key_mgmt = WPA_KEY_MGMT_FT_IEEE8021X_SHA384;
// wpa_dbg(wpa_s, MSG_DEBUG, 			"WPA: using KEY_MGMT
// FT/802.1X-SHA384"); 		if
...
// 			wpa_dbg(wpa_s, MSG_DEBUG,
// 				"WPA: Disable PMKSA caching for FT/802.1X
// connection"); 			pmksa_cache_clear_current(wpa_s->wpa);
// 		}
// #endif /* CONFIG_SHA384 */
// #endif /* CONFIG_IEEE80211R */
...
// 		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT
// FT-FILS-SHA384"); 	} else if (sel & WPA_KEY_MGMT_FT_FILS_SHA256) {
// 		wpa_s->key_mgmt = WPA_KEY_MGMT_FT_FILS_SHA256;
// 		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT
// FT-FILS-SHA256"); #endif /* CONFIG_IEEE80211R */ 	} else if (sel &
// WPA_KEY_MGMT_FILS_SHA384) { 		wpa_s->key_mgmt =
// WPA_KEY_MGMT_FILS_SHA384; 		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using
// KEY_MGMT FILS-SHA384"); 	} else if (sel & WPA_KEY_MGMT_FILS_SHA256) {
// wpa_s->key_mgmt = WPA_KEY_MGMT_FILS_SHA256; 		wpa_dbg(wpa_s,
// MSG_DEBUG, "WPA: using KEY_MGMT FILS-SHA256"); #endif /* CONFIG_FILS */
// #ifdef CONFIG_IEEE80211R 	} else if
// ((sel & WPA_KEY_MGMT_FT_IEEE8021X) &&
// 		   os_strcmp(wpa_supplicant_get_eap_mode(wpa_s), "LEAP") != 0)
// { 		wpa_s->key_mgmt = WPA_KEY_MGMT_FT_IEEE8021X;
// wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using KEY_MGMT FT/802.1X"); 		if
// (!ssid->ft_eap_pmksa_caching &&
// pmksa_cache_get_current(wpa_s->wpa)) {
// 			/* PMKSA caching with FT may have interoperability
// 			 * issues, so disable that case by default for now. */
...
// 	    wpas_get_ssid_pmf(wpa_s, ssid) == MGMT_FRAME_PROTECTION_REQUIRED) {
// 		wpa_msg(wpa_s, MSG_INFO,
// 			"RSN: Management frame protection required but the
// selected AP does not enable it"); 		return -1;
// 	}
...
// 		sel = 0;
// 	wpa_dbg(wpa_s, MSG_DEBUG,
// 		"WPA: AP mgmt_group_cipher 0x%x network profile
// mgmt_group_cipher 0x%x; available mgmt_group_cipher 0x%x",
// ie.mgmt_group_cipher, ssid->group_mgmt_cipher, sel); 	if (sel &
// WPA_CIPHER_AES_128_CMAC) { 		wpa_s->mgmt_group_cipher =
// WPA_CIPHER_AES_128_CMAC; 		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using
// MGMT group
// cipher " 			"AES-128-CMAC"); 	} else if (sel &
// WPA_CIPHER_BIP_GMAC_128) { 		wpa_s->mgmt_group_cipher =
// WPA_CIPHER_BIP_GMAC_128; 		wpa_dbg(wpa_s, MSG_DEBUG, "WPA: using MGMT group
// cipher " 			"BIP-GMAC-128"); 	} else if (sel & WPA_CIPHER_BIP_GMAC_256) {
...
// 		wpa_msg(wpa_s, MSG_INFO,
// 			"Disable PTK0 rekey support - replaced with
// reconnect"); 		wpa_s->deny_ptk0_rekey = 1;
// wpa_sm_set_param(wpa_s->wpa, WPA_PARAM_DENY_PTK0_REKEY, 1); 	} else {
// wpa_s->deny_ptk0_rekey = 0; 		wpa_sm_set_param(wpa_s->wpa,
// WPA_PARAM_DENY_PTK0_REKEY, 0);
// 	}

// 	return 0;
// }

// static void wpas_ext_capab_byte(struct wpa_supplicant *wpa_s, u8 *pos, int
// idx)
// {
// 	*pos = 0x00;
...
// int wpas_build_ext_capab(struct wpa_supplicant *wpa_s, u8 *buf, size_t
// buflen)
// {
// 	u8 *pos = buf;
// 	u8 len = 11, i;
...
// 		wpa_printf(MSG_INFO,
// 			   "Not enough room for building extended capabilities
// element"); 		return -1;
// 	}
...
// int wpas_valid_bss_ssid(struct wpa_supplicant *wpa_s, struct wpa_bss
// *test_bss, 			struct wpa_ssid *test_ssid)
// {
...
// 		wpa_msg(wpa_s, MSG_DEBUG,
// 			"Previously selected random MAC address has not yet
// expired"); 		return 0;
// 	}
...
// static void wpa_s_setup_sae_pt(struct wpa_config *conf, struct wpa_ssid
// *ssid)
// {
// #ifdef CONFIG_SAE
...
// 				   "TESTING: Indicate rejection of an extra SAE
// group %d", 				   groups[i]);
...
// 	/*
// 	 * Set WPA state machine configuration to match the selected network
// now
...
// 		wpa_dbg(wpa_s, MSG_DEBUG, "Reject wpa_supplicant_associate()
// call since connect_work exist"); 		return;
// 	}

// 	if (radio_work_pending(wpa_s, "connect")) {
// 		wpa_dbg(wpa_s, MSG_DEBUG, "Reject wpa_supplicant_associate()
// call since pending work exist"); 		return;
// 	}
...
// 			wpa_printf(MSG_DEBUG,
// 				   "IBSS already found in scan results, adjust
// control freq: %d", 				   bss->freq);
// freq->freq = bss->freq; 			obss_scan = 0;
// break;
// 		}
// 	}
...
// #ifdef CONFIG_FILS
// static size_t wpas_add_fils_hlp_req(struct wpa_supplicant *wpa_s, u8
// *ie_buf, 				    size_t ie_buf_len)
// {
// 	struct fils_hlp_req *req;
...
// 				   "FILS: Cannot fit HLP - rem_len=%lu
// to_fill=%lu", 				   (unsigned long) rem_len,
// (unsigned long) (2 + hdr_len + hlp_len)); 			break;
// 		}
...
// 	if (!wpa_ie) {
// 		wpa_printf(MSG_ERROR,
// 			   "Failed to allocate connect IE buffer for %lu
// bytes", 			   (unsigned long) max_wpa_ie_len);
// return NULL;
// 	}
...
// 		wpa_dbg(wpa_s, MSG_DEBUG,
// 			"SAE: Use WPA_AUTH_ALG_OPEN for PMKSA caching
// attempt"); 		algs = WPA_AUTH_ALG_OPEN;
// 	}
// #endif /* CONFIG_SAE */
...
// 	wpa_drv_update_connect_params(wpa_s, &params,
// WPA_DRV_UPDATE_ASSOC_IES); 	os_free(wpa_ie);
// }
// #endif /* CONFIG_OWE */
...
// 	if (!result.channels) {
// 		wpa_printf(MSG_DEBUG,
// 			   "EDMG not possible: cannot intersect channels 0x%x
// and 0x%x", 			   a.channels, b.channels); 		goto
// fail;
// 	}

// 	if (!(result.channels & BIT(primary_channel - 1))) {
// 		wpa_printf(MSG_DEBUG,
// 			   "EDMG not possible: the primary channel %d is not
// one of the intersected channels 0x%x", primary_channel, result.channels);
// goto fail;
// 	}
...
// 		wpa_printf(MSG_DEBUG,
// 			   "EDMG not possible: not enough contiguous channels
// %d for supporting CB1 or CB2", 			   max_contiguous);
// goto fail;
// 	}
...
// 	return get_edmg_intersection(mode->edmg, request_edmg,
// primary_channel);
...
// 		wpa_dbg(wpa_s, MSG_DEBUG, "BSS/SSID entry for association not
// valid anymore - drop connection attempt");
// wpas_connect_work_done(wpa_s); 		return;
// 	}
...
// 				wpa_printf(MSG_DEBUG,
// 					   "Use EDMG channel %d for
// connection", 					   ssid->edmg_channel);
// } else { 			edmg_fail:
// params.freq.edmg.channels = 0; 				params.freq.edmg.bw_config = 0;
// 				wpa_printf(MSG_WARNING,
// 					   "EDMG channel %d not supported by
// AP, fallback to DMG", ssid->edmg_channel);
// 			}
// 		}
...
// ssid->proactive_key_caching; 		else 			params.req_key_mgmt_offload = 1;
...
// 			   !ssid->owe_only) {
// 			params.mgmt_frame_protection =
// NO_MGMT_FRAME_PROTECTION; #endif /* CONFIG_OWE */
// 		}
// 	}
...
//  *
//  * This function is used to request %wpa_supplicant to deauthenticate from
//  the
//  * current AP.
//  */
...
//  * Returns: 0 on success, or -1 if the network was not found, -2 if the
//  network
...
// 		wpa_printf(MSG_DEBUG,
// 			   "Stop ongoing sched_scan to remove network from
// filters"); 		wpa_supplicant_cancel_sched_scan(wpa_s);
// 		wpa_supplicant_req_scan(wpa_s, 0, 0);
// 	}
...
//  * @wpa_s: wpa_supplicant structure for a network interface
//  * @ssid: wpa_ssid structure for a configured network or %NULL for any
//  network
//  */
...
// /**
//  * wpas_set_pkcs11_engine_and_module_path - Set PKCS #11 engine and module
//  path
//  * @wpa_s: wpa_supplicant structure for a network interface
...
//  * path. If resetting the EAPOL state machine with the new PKCS #11 engine
//  and
...
// 		wpa_printf(MSG_INFO,
// 			   "Note: nl80211 driver interface is not designed to
...
// /**
//  * wpa_supplicant_set_bss_expiration_count - Set BSS entry expiration scan
//  count
//  * @wpa_s: wpa_supplicant structure for a network interface
...
//  */
// int wpa_supplicant_set_debug_params(struct wpa_global *global, int
// debug_level, 				    int debug_timestamp, int
// debug_show_keys)
// {
...
// #ifdef CONFIG_OWE
// static int owe_trans_ssid_match(struct wpa_supplicant *wpa_s, const u8
// *bssid, 				const u8 *entry_ssid, size_t
// entry_ssid_len)
// {
...
// /**
//  * wpa_supplicant_rx_eapol - Deliver a received EAPOL frame to
//  wpa_supplicant
//  * @ctx: Context pointer (wpa_s); this is the ctx variable registered
//  *	with struct wpa_driver_ops::init()
//  * @src_addr: Source address of the EAPOL frame
//  * @buf: EAPOL data starting from the EAPOL header (i.e., no Ethernet
//  header)
//  * @len: Length of the EAPOL data
//  *
//  * This function is called for each received EAPOL frame. Most driver
//  * interfaces rely on more generic OS mechanism for receiving frames through
//  * l2_packet, but if such a mechanism is not available, the driver wrapper
//  may
//  * take care of received EAPOL frames and deliver them to the core
//  supplicant
...
// 		wpa_printf(MSG_DEBUG,
// 			   "Drop received EAPOL frame as we are
...
// 		 * the association event is received. This may also be needed
// in
// 		 * driver-based roaming case, so also use src_addr != BSSID as
// a
...
// 			/*
// 			 * Use shorter timeout if going through WPS AP
// iteration
...
// 			if (wps_ie &&
// 			    !wps_is_addr_authorized(wps_ie, wpa_s->own_addr,
// 1)) 				timeout = 10;
// wpabuf_free(wps_ie);
// 		}
// #endif /* CONFIG_WPS */
...
// 		if (!wpa_s->l2_br) {
// 			wpa_msg(wpa_s, MSG_ERROR,
// 				"Failed to open l2_packet connection for the
// bridge interface '%s'", 				wpa_s->bridge_ifname);
// goto fail;
// 		}
// 	}
...
//  * wpa_drv_init() must have been called before this function to initialize
//  the
//  * driver interface.
//  */
...
// 		wpa_msg(wpa_s, MSG_ERROR,
// 			"tx_stbc: %d out of range. Must be 0-1 or -1",
// tx_stbc); 		return -EINVAL;
// 	}
...
// 		wpa_msg(wpa_s, MSG_ERROR,
// 			"rx_stbc: %d out of range. Must be 0-3 or -1",
// rx_stbc); 		return -EINVAL;
// 	}
...
// host_to_le32(ssid->vht_capa_mask);
...
// static int wpas_fst_send_action_cb(void *ctx, const u8 *da, struct wpabuf
// *data)
// {
// 	struct wpa_supplicant *wpa_s = ctx;
...
// static const struct wpabuf * wpas_fst_get_mb_ie_cb(void *ctx, const u8
// *addr)
// {
// 	struct wpa_supplicant *wpa_s = ctx;
...
// 			wpa_printf(MSG_DEBUG, "Add interface %s to existing
// radio %s", 				   wpa_s->ifname, rn);
// dl_list_add(&radio->ifaces, &wpa_s->radio_list); 			return
// radio;
// 		}
...
// 		wpa_dbg(work->wpa_s, MSG_INFO, "Freeing radio work '%s'@%p
// (started=%d) that is marked as scan_work", 			work->type,
// work, work->started); 		work->wpa_s->scan_work = NULL;
// 	}
...
// 		/* This should not really happen. */
// 		wpa_dbg(work->wpa_s, MSG_INFO, "Freeing radio work '%s'@%p
// (started=%d) that is marked as p2p_scan_work",
// work->type, work, work->started); 		work->wpa_s->p2p_scan_work =
// NULL;
// 	}
// #endif /* CONFIG_P2P */
...
// static struct wpa_radio_work * radio_work_get_next_work(struct wpa_radio
// *radio)
// {
...
// 			wpa_dbg(active_work->wpa_s, MSG_DEBUG,
// 				"Do not start work '%s' when another work '%s'
// is
// already scheduled", 				tmp->type, active_work->type);
// continue;
// 		}
// 		/*
...
// 			 * interface and if an external scan is already
// running,
...
// 			wpa_printf(MSG_DEBUG, "Delay radio work start until
// externally triggered scan completes"); 			return;
// 		}
// 	} else {
// 		work = NULL;
...
// 		wpa_printf(MSG_DEBUG,
// 			   "External radio work in progress - delay start of
// pending item"); 		return;
// 	}
// 	eloop_cancel_timeout(radio_start_next_work, radio, NULL);
...
//  * exclusive radio control. Once the radio is available, the registered
//  callback
//  * function will be called. radio_work_done() must be called once the
//  exclusive
...
//  * during interface removal. That does not allow the callback function to
//  start
//  * the radio operation, i.e., it must free any resources allocated for the
//  radio
//  * work and return.
//  *
...
//  * operations to be performed in parallel if they apply for the same
//  channel.
//  * Setting this to 0 indicates that the work item may use multiple channels
//  or
...
// 	if (was_empty) {
// 		wpa_dbg(wpa_s, MSG_DEBUG, "First radio work item in the queue -
// schedule start immediately"); 		radio_work_check_next(wpa_s);
// } else if
// ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_OFFCHANNEL_SIMULTANEOUS)
// 		   && radio->num_active_works < MAX_ACTIVE_WORKS) {
// 		wpa_dbg(wpa_s, MSG_DEBUG,
...
// 			wpa_printf(MSG_ERROR,
// 				   "OCE STA-CFON feature is not yet
// supported");
// 		}
// 	}
...
// 		}
// 		wpa_printf(MSG_DEBUG,
// 			   "Remove remaining child interface %s from parent
// %s", 			   iface->ifname, wpa_s->ifname); 		prev
...
// 		} else {
// 			wpa_msg(wpa_s, MSG_INFO,
// 				"Do not deauthenticate as part of interface
// deinit since WoWLAN is enabled");
// 		}
// 	}
...
//  * This can be called before wpa_supplicant_run() to add interfaces before
//  the
//  * main event loop has been started. In addition, new interfaces can be
//  added
...
//  * there are any remaining events. In most cases, this function is running
//  as
...
// 		wpas_sched_scan_plans_set(wpa_s,
// wpa_s->conf->sched_scan_plans);
...
// 		wpa_s->own_disconnect_req = 0;
// 		wpa_dbg(wpa_s, MSG_DEBUG,
// 			"Ignore connection failure due to local request to
// disconnect"); 		return;
// 	}
...
// 	wpa_dbg(wpa_s, MSG_DEBUG,
// 		"Consecutive connection failures: %d --> request scan in %d
// ms", 		wpa_s->consecutive_conn_failures, timeout);
...
// int wpas_network_disabled(struct wpa_supplicant *wpa_s, struct wpa_ssid
// *ssid)
// {
// #ifdef CONFIG_WEP
// 	int i;
...
// 			/*
// 			 * Do not use the default PMF value for non-RSN
// networks
...
//  * @wpa_s: Pointer to the network interface
//  *
//  * This function is used to request a new connection to be found. It will
//  mark
...
//  * @wpa_s: Pointer to the network interface
//  *
//  * This function is used to request disconnection from the currently
//  connected
...
// struct wpa_supplicant *
// wpas_vendor_elem(struct wpa_supplicant *wpa_s, enum wpa_vendor_elem_frame
// frame)
// {
// 	switch (frame) {
// #ifdef CONFIG_P2P
...
// static struct
// wpa_bss_tmp_disallowed * wpas_get_disallowed_bss(struct wpa_supplicant
...
// 		if (wpas_mac_addr_rand_scan_set(wpa_s,
// MAC_ADDR_RAND_SCHED_SCAN, 						addr,
// mask)) return -1;
...
// 				continue;
// 			wpa_printf(MSG_DEBUG,
// 				   "Override driver signal_poll information:
// current_signal: %d->%d avg_signal: %d->%d avg_beacon_signal: %d->%d
// current_noise: %d->%d", 				   si->current_signal,
// dso->si_current_signal, 				   si->avg_signal,
// dso->si_avg_signal, 				   si->avg_beacon_signal, 				   dso->si_avg_beacon_signal,
// 				   si->current_noise,
...
// 				continue;
// 			wpa_printf(MSG_DEBUG,
// 				   "Override driver scan signal level %d->%d
// for
// " 				   MACSTR, res->level, dso->scan_level,
// MAC2STR(res->bssid)); res->flags |= WPA_SCAN_QUAL_INVALID; if
// (dso->scan_level < 0) 				res->flags |=
// WPA_SCAN_LEVEL_DBM; 			else 				res->flags
// &= ~WPA_SCAN_LEVEL_DBM; 			res->level = dso->scan_level;
// break;
// 		}
// 	}
// #endif /* CONFIG_TESTING_OPTIONS */
